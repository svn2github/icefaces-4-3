...
if (!ice.ace.ListExporters) ice.ace.ListExporters = {};

ice.ace.ListExporter = function(id, behaviors) {
	this.id = id;
	if (!ice.ace.ListExporters[this.id]) {
		ice.ace.ListExporters[this.id] = this;
		if (ice.ace.ListExporter.shouldOpenPopUp()) {
			if (!this.window) {
				this.openWindow();
			}
		}
		...
		if (ice.ace.ListExporter.shouldOpenPopUp()) {
			var instance = ice.ace.ListExporters[this.id];
...
ice.ace.ListExporter.prototype.openWindow = function() {
	this.window = window.open('','','width=400,height=150');
	this.window.document.write('<html><head><title>List export file loader</title></head><body id="body"></body></html>');
	this.body = this.window.document.getElementById('body');
	this.body.innerHTML = '<p>Please wait while your file is generated...</p>';
};
ice.ace.ListExporter.prototype.url = function(url) {
	if (ice.ace.ListExporter.shouldOpenPopUp()) {
...
ice.ace.ListExporter.shouldOpenPopUp = function() {
