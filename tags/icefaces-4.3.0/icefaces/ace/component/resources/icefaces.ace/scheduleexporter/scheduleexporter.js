...
if (!ice.ace.ScheduleExporters) ice.ace.ScheduleExporters = {};

ice.ace.ScheduleExporter = function(id, behaviors) {
	this.id = id;
	if (!ice.ace.ScheduleExporters[this.id]) {
		ice.ace.ScheduleExporters[this.id] = this;
		if (ice.ace.ScheduleExporter.shouldOpenPopUp()) {
			if (!this.window) {
				this.openWindow();
			}
		}
		...
		if (ice.ace.ScheduleExporter.shouldOpenPopUp()) {
			var instance = ice.ace.ScheduleExporters[this.id];
...
ice.ace.ScheduleExporter.prototype.openWindow = function() {
	this.window = window.open('','','width=400,height=150');
	this.window.document.write('<html><head><title>Schedule export file loader</title></head><body id="body"></body></html>');
	this.body = this.window.document.getElementById('body');
	this.body.innerHTML = '<p>Please wait while your file is generated...</p>';
};
ice.ace.ScheduleExporter.prototype.url = function(url) {
	if (ice.ace.ScheduleExporter.shouldOpenPopUp()) {
...
ice.ace.ScheduleExporter.shouldOpenPopUp = function() {
